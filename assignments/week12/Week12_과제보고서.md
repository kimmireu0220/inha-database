...
### 1.1 과제 목표
본 과제는 11주차에서 구현한 Inha DB 웹 애플리케이션에 수강신청 기능을 추가하는 것임. 로그인한 학생이 'enrollment' 메뉴를 클릭하면 수강신청 페이지로 이동하고, Apply 버튼을 통해 수강신청을 할 수 있도록 구현했음. 또한 수강신청 시 제약 조건(이미 수강신청한 수업은 신청 불가, Remaining participants가 0이면 신청 불가)을 적용하여 데이터 무결성을 보장했음.

### 1.2 구현 요구사항
- **STEP 1**: 로그인 했을 경우에 'enrollment'를 클릭하면 수강신청 페이지로 이동
  - 로그인한 학생의 학번을 쿠키로 저장하여 인증 상태 유지
  - enrollment 메뉴 클릭 시 `/sugang` 경로로 이동하여 수강신청 페이지 표시
...
- "enrollment" 메뉴를 클릭하면 수강신청 페이지로 이동함.
...
**설명**:
- enrollment 메뉴를 클릭하여 이동한 수강신청 페이지임.
...
**STEP 1: 수강신청 페이지 이동**
- ✅ 로그인한 학생이 'enrollment' 메뉴를 클릭하면 수강신청 페이지로 이동하는 기능 구현 완료
