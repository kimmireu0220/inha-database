...
위 EER diagram은 MySQL Workbench에서 `Database` > `Reverse Engineer` 기능을 사용하여 생성했음. AIRLINE 데이터베이스의 모든 10개 테이블과 외래키 관계가 시각적으로 표현되어 있으며, 각 테이블의 Primary Key(노란색 열쇠 아이콘)와 Foreign Key(빨간색 열쇠 아이콘) 관계가 명확히 표시되어 있음. 또한 각 관계선에는 카디널리티(1:N, N:M 등)가 표시되어 있어 테이블 간의 관계를 쉽게 파악할 수 있음.
