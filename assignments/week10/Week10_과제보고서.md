...
### 1.1 과제 목표
본 과제는 7주차에서 구현한 Inha DB 웹 애플리케이션에 로그인 기능과 수강취소 기능을 추가하는 것임. 학생의 학번과 전화번호를 이용한 로그인 시스템을 구현했고, 로그인한 학생만 자신의 정보를 조회하고 수강 중인 수업을 수강취소할 수 있도록 권한 관리를 적용했음.
...
학생과 수업 간의 관계를 나타내기 위해 `Enrollment` 테이블을 생성했음.
...
**설계 사고 과정**: 
- Student와 Class 테이블은 이미 존재했지만, 학생이 어떤 수업을 수강하는지 나타내는 관계 테이블이 없었음.
- 다대다 관계를 표현하기 위해 Enrollment 테이블을 생성하여 Student_Id와 Class_Id를 외래키로 사용했음.
- CASCADE 옵션을 사용하여 학생이나 수업이 삭제될 때 관련 수강 정보도 자동으로 삭제되도록 설계했음.
...
**알고리즘 동작 과정**:
1. 사용자가 로그인 폼에서 학번과 비밀번호(전화번호)를 입력함.
2. POST 요청이 들어오면 `getStudentByLogin()` 함수를 호출하여 Student 테이블에서 해당 학번과 전화번호가 일치하는 학생을 조회함.
3. 학생 정보가 존재하면 세션에 사용자 정보를 저장함 (`id`, `role: 'student'`, `checkLogin: true`).
4. 로그인 성공 시 `/select` 페이지로 리다이렉트함.
...
`database/sql.js`에 로그인 관련 쿼리 함수를 추가했음.

**설계 사고 과정**:
- 학번(Id)과 전화번호(PhoneNumber)를 동시에 조건으로 사용하여 정확한 학생을 찾음.
- SQL Injection을 방지하기 위해 나중에 prepared statement로 개선할 수 있지만, 과제 요구사항에 맞게 기본적인 구현을 완료했음.

#### 2.2.3 로그인 페이지 구현
`views/login.hbs` 파일을 생성하여 로그인 폼을 구현했음.

**주요 특징**:
- 학번 입력 필드: `name="id"` 속성으로 학번을 입력받음.
- 비밀번호 입력 필드: `name="password"` 속성으로 전화번호를 입력받음.
- Week10의 로그인 페이지 스타일을 참고하여 일관된 UI를 제공했음.
...
**알고리즘 동작 과정**:
1. 페이지 접속 시 세션에 로그인 정보가 있는지 확인함.
2. 로그인하지 않은 경우 `/`로 리다이렉트함.
3. 로그인한 경우 세션에서 학생 ID를 가져옴.
4. `getStudentById()` 함수를 사용하여 해당 학생의 정보만 조회함.
5. 전체 테이블 정보도 함께 조회하여 기존 기능을 유지함.

**구현 방식과 사고 과정**:
- 기존에는 모든 학생 정보를 조회했지만, 로그인한 학생의 정보만 표시하도록 변경했음.
- 다른 테이블 정보는 그대로 유지하여 전체 데이터베이스 구조를 확인할 수 있도록 했음.
- 템플릿 호환성을 위해 단일 학생 객체를 배열로 변환했음.
...
**알고리즘 동작 과정**:
1. GET `/delete/class`: 
   - 세션에서 로그인 정보와 학생 권한을 확인함.
   - 로그인하지 않았거나 학생 권한이 아니면 로그인 페이지로 리다이렉트함.
   - 로그인한 학생의 ID를 가져와 `getEnrolledClasses()` 함수로 수강 중인 수업 목록을 조회함.
   - 수강 수업 목록을 템플릿에 전달하여 화면에 표시함.

2. POST `/delete/class`:
   - 권한을 다시 한 번 확인함.
   - 요청 본문에서 `classId`를 가져옴.
   - `deleteEnrollment()` 함수를 호출하여 Enrollment 테이블에서 해당 레코드를 삭제함.
...
**설계 사고 과정**:
- `getEnrolledClasses()`: Enrollment 테이블과 Class 테이블을 INNER JOIN하여 해당 학생이 수강하는 수업의 상세 정보를 가져옴.
- `deleteEnrollment()`: Student_Id와 Class_Id를 모두 조건으로 사용하여 정확한 수강 정보만 삭제함.
- 두 조건을 모두 사용함으로써 다른 학생의 수강 정보를 실수로 삭제하는 것을 방지함.

#### 2.3.3 수강취소 페이지 구현
`views/deleteClass.hbs` 파일을 생성하여 수강취소 페이지를 구현했음.

**주요 특징**:
- 수강 중인 수업 목록을 테이블 형태로 표시함.
- 각 수업마다 "수강취소" 버튼을 제공함.
- 버튼 클릭 시 해당 수업의 ID가 hidden input으로 전송됨.
- 수강 중인 수업이 없을 경우 안내 메시지를 표시함.
...
**설계 사고 과정**:
- `secret`: 세션 쿠키를 암호화하기 위한 비밀키임.
- `resave`: 세션이 수정되지 않아도 저장하도록 설정했음.
- `saveUninitialized`: 초기화되지 않은 세션도 저장하도록 설정했음.
- 세션을 통해 로그인 상태를 유지하고, 각 페이지에서 권한을 체크할 수 있도록 했음.
...
- 로그인한 학생(학생 2번: Hong Gil Dong)의 정보가 "Student (내 정보)" 섹션에 표시됨.
...
- Database 수업이 삭제되어 Circuit Theory 수업만 남아있음.
...
- 알림 창을 닫으면 다시 로그인 페이지로 돌아옴.
- 보안을 위해 구체적인 오류 메시지는 표시하지 않음.
...
1. **데이터베이스 설계**: Student와 Class 간의 관계를 나타내기 위해 Enrollment 테이블을 생성했음.
2. **보안**: 세션을 통한 권한 관리로 로그인하지 않은 사용자의 접근을 차단했음.
3. **사용자 경험**: 로그인 실패 시 명확한 피드백을 제공하고, 수강취소 후 즉시 업데이트된 목록을 표시했음.
4. **에러 처리**: try-catch 블록을 사용하여 예외 상황을 처리하고 사용자에게 알림을 제공했음.

### 4.3 개선 가능한 사항
1. **SQL Injection 방지**: 현재는 문자열 연결을 사용하지만, prepared statement를 사용하여 보안을 강화할 수 있음.
2. **비밀번호 암호화**: 현재는 전화번호를 평문으로 저장하지만, 실제 환경에서는 해시 함수를 사용하여 암호화해야 함.
3. **세션 만료 시간 설정**: 현재는 세션 만료 시간이 설정되어 있지 않으므로, 적절한 만료 시간을 설정할 수 있음.
4. **로깅 시스템**: 더 체계적인 로깅 시스템을 구축하여 디버깅과 모니터링을 개선할 수 있음.

### 4.4 최종 평가
본 과제를 통해 웹 애플리케이션에 로그인 기능과 권한 관리를 추가하는 방법을 학습했음. Express.js의 세션 관리 기능을 활용하여 사용자 인증과 권한 제어를 구현했고, 데이터베이스 관계를 이해하고 Enrollment 테이블을 통해 학생과 수업 간의 관계를 표현했음. 모든 요구사항을 충족하는 완전한 기능을 구현했고, 실제 테스트를 통해 정상 작동을 확인했음.
