...
### 1.1 과제 목표
7주차에서 구현한 Inha DB 웹 애플리케이션에 로그인 기능과 수강취소 기능을 추가하는 과제임. 학생의 학번과 전화번호를 이용한 로그인 시스템을 구현하고, 로그인한 학생만 자신의 정보를 조회하고 수강 중인 수업을 수강취소할 수 있도록 권한 관리를 적용했음.
...
3. 학생 정보가 존재하면 세션에 사용자 정보를 저장함.
4. 로그인 성공 시 `/select` 페이지로 리다이렉트함.
5. 학생 정보가 없으면 경고 창을 띄우고 로그인 페이지로 다시 이동함.

**구현 방식과 사고 과정**:
- Student 테이블의 Id와 Phone_number를 직접 사용하여 로그인을 구현했음.
...
**알고리즘 동작 과정**:
1. GET `/delete/class`: 세션에서 로그인 정보와 학생 권한을 확인하고, 수강 중인 수업 목록을 조회하여 화면에 표시함.
2. POST `/delete/class`: 권한을 확인한 후 Enrollment 테이블에서 해당 레코드를 삭제하고, 수강취소 페이지로 다시 리다이렉트함.
...
- 로그인한 학생(학생 1번: LEE Sangwon)의 정보가 "Student (내 정보)" 섹션에 표시됨.
...
**설명**:
- Database Systems 수업의 "수강취소" 버튼을 클릭한 후의 화면임.
- Database Systems 수업이 삭제되어 Thermodynamics 수업만 남아있음.
...
### 4.3 최종 평가
본 과제를 통해 웹 애플리케이션에 로그인 기능과 권한 관리를 추가하는 방법을 학습했음. Express.js의 세션 관리 기능을 활용하여 사용자 인증과 권한 제어를 구현했으며, 데이터베이스 관계를 이해하고 Enrollment 테이블을 통해 학생과 수업 간의 관계를 표현했음. 모든 요구사항을 충족하는 완전한 기능을 구현했으며, 실제 테스트를 통해 정상 작동을 확인했음.
